...
import type { CurrentUser, StructuredBlock, StructuredTopic, StudyHighlight } from '../../types'
...
// ── HighlightedText ───────────────────────────────────────────────────────
interface TextSegment { text: string; type: string | null }
...
function HighlightedText({ text, keywords = [], laws = [], dates = [] }: {
  ...
  ].filter(({ term }) => term && term.length >= 6).sort((a, b) => b.term.length - a.term.length)
  ...
  return (
    <p className={styles.paragraph}>
      {parts.map((part, i) => {
        ...
        let segments: TextSegment[] = [{ text: part, type: null }]
...
// ── Segment types ─────────────────────────────────────────────────────────
interface MarkSegment { type: 'mark'; text: string; color: string; id: string }
interface PlainSegment { type: 'text'; text: string }
type Segment = MarkSegment | PlainSegment

function applyMarksToSection(sectionText: string, highlights: StudyHighlight[]): Segment[] | null {
...
function renderMdText(text: string): React.ReactNode[] {
  const parts = text.split(/(\*\*[^*]+\*\*)/)
...
interface HighlightableContentProps {
  ...
  onAdd:          (args: { topicId: string; startOffset: number; endOffset: number; text: string; color: string }) => Promise<StudyHighlight | null>
...
function HighlightableContent({ topic, highlights, onAdd, onRemove, highlightMode }: HighlightableContentProps) {
  ...
  const handlePickColor = useCallback(async (colorEntry: typeof HIGHLIGHT_COLORS[number], e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (!text) return
    await onAdd({ topicId: topic.id, startOffset: 0, endOffset: 0, text, color: colorEntry.bg })
              ...
              return (
                <li key={j}>
                  {itemSegs
                    ? itemSegs.map((s, k) =>
                        s.type === 'mark'
                          ? <mark key={k} className={styles.hlMark} style={{ background: s.color }} onClick={() => onRemove(s.id)} title="Clic para eliminar">{s.text}</mark>
                          : renderMdText(s.text)
                      )
  ...
  return (
    ...
    >
      {tooltip && highlightMode && (
        <div className={styles.colorPicker} style={{ left: tooltip.x, top: tooltip.y }}
          onMouseDown={e => e.preventDefault()} onTouchStart={e => e.preventDefault()} onClick={e => e.stopPropagation()}>
          <span className={styles.colorPickerLabel}>Color:</span>
          {HIGHLIGHT_COLORS.map(c => (
            <button key={c.id} className={styles.colorDot} style={{ background: c.bg }}
              onMouseDown={e => { e.preventDefault(); e.stopPropagation() }}
              onTouchStart={e => { e.preventDefault(); e.stopPropagation() }}
              onClick={e => handlePickColor(c, e)} title={c.label} />
          ))}
        </div>
      )}
      {highlights.length > 0
        ...
        <div className={styles.highlightList}>
          ...
          {highlights.map(h => (
            <div key={h.id} className={styles.highlightPill} style={{ background: h.color }}>
              <span className={styles.highlightPillText}>{h.text.length > 60 ? h.text.slice(0, 60) + '…' : h.text}</span>
...
function MisApuntes({ highlights, blocks, onRemove }: {
  ...
  const byBlock: Record<string, { block: StructuredBlock; topics: Record<string, { topic: StructuredTopic; hl: StudyHighlight[] }> }> = {}
      ...
      {Object.values(byBlock).map(({ block, topics }) => (
        <div key={block.id} className={styles.apuntesBlock}>
          <div className={styles.apuntesBlockHeader} style={{ color: block.color }}>
            <BookOpen size={13} strokeWidth={2} /><span>{block.label}</span>
          </div>
              ...
              <div className={styles.apuntesFragments}>
                {hl.map(h => (
                  <div key={h.id} className={styles.apuntesFragment}>
                    <mark className={styles.apuntesMark} style={{ background: h.color }}>"{h.text}"</mark>
...
// ── ReadMode ──────────────────────────────────────────────────────────────
interface ReadModeState { block: StructuredBlock; topicIndex: number }

interface ReadModeProps {
  block:              StructuredBlock
  topicIndex:         number
  onClose:            () => void
  onToggleRead:       (topicId: string, blockId: string) => void
  onToggleBookmark:   (topicId: string, blockId: string) => void
  readTopics:         Set<string>
  bookmarks:          Set<string>
  onTest:             (blockId: string, blockLabel: string, topicId: string, topicTitle: string) => void
  highlights:         StudyHighlight[]
  onAddHighlight:     HighlightableContentProps['onAdd']
  onRemoveHighlight:  (id: string) => void
}

function ReadMode({
  block, topicIndex, onClose, onToggleRead, onToggleBookmark,
    ...
    <div className={styles.readOverlay}>
        ...
        <div className={styles.readProgressDots}>
          {block.topics.map((t, i) => (
            <button key={t.id}
              ...
              style={(i===idx||readTopics.has(t.id))?{['--bc' as string]:block.color}:{}}
              onClick={() => setIdx(i)} title={t.title} />
          ))}
        </div>
        <div className={styles.readTopRight}>
          <button className={[styles.readActionBtn, isBookmarked ? styles.readActionActive : ''].join(' ')}
            style={isBookmarked ? { color: block.color } : {}}
            onClick={() => onToggleBookmark(topic.id, block.id)}>
            {isBookmarked ? <BookmarkCheck size={16} /> : <Bookmark size={16} />}
          </button>
          <button className={[styles.highlightToggleBtn, highlightMode ? styles.highlightToggleBtnActive : ''].join(' ')} onClick={() => setHL(v => !v)}>
            <Highlighter size={13} /> {highlightMode ? 'Listo' : 'Subrayar'}
          </button>
          {topicHL.length > 0 && <span className={styles.highlightBadge}><Highlighter size={11} /> {topicHL.length}</span>}
      ...
      <div className={styles.readScroll} ref={scrollRef}>
        <div className={styles.readContent}>
          ...
          {highlightMode && (
            <div className={styles.highlightHint}><Highlighter size={11} /> Selecciona el texto que quieres subrayar</div>
          )}

          <HighlightableContent topic={topic} highlights={topicHL} onAdd={onAddHighlight} onRemove={onRemoveHighlight} highlightMode={highlightMode} />
...
interface StudyViewProps {
  ...
  onSelectMode:   (blockId: string, blockLabel: string, topicId?: string, topicTitle?: string) => void
...
export default function StudyView({ currentUser, onSelectMode, initialBlockId }: StudyViewProps) {
  ...
  const [readMode,  setReadMode]  = useState<ReadModeState | null>(null)
  const [expanded,  setExpanded]  = useState<Record<string, boolean>>({})
  const [activeTab, setActiveTab] = useState<'temario' | 'apuntes'>('temario')
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.studyTabs}>
        <button className={[styles.studyTab, activeTab==='temario'?styles.studyTabActive:''].join(' ')} onClick={() => setActiveTab('temario')}>
          <BookOpen size={14} /> Temario
        </button>
        <button className={[styles.studyTab, activeTab==='apuntes'?styles.studyTabActive:''].join(' ')} onClick={() => setActiveTab('apuntes')}>
          <Highlighter size={14} /> Mis apuntes
          {highlights.length > 0 && <span className={styles.studyTabBadge}>{highlights.length}</span>}
        </button>
      </div>

      {activeTab === 'apuntes' && <MisApuntes highlights={highlights} blocks={blocks} onRemove={removeHighlight} />}
            ...
            {blocks.map(block => {
              ...
              return (
                <div key={block.id} id={`block-${block.id}`}
                  ...
                  style={pct===100?{['--done-color' as string]:block.color}:{}}>

                  <button className={styles.blockRowHeader} onClick={() => toggleExpand(block.id)} style={{['--bc' as string]:block.color}}>
                    ...
                    <div className={styles.blockRowRight}>
                      ...
                      <ChevronRight size={16} className={styles.blockRowChevron} style={{ transform: isOpen?'rotate(90deg)':'rotate(0deg)' }} />
                    </div>
                  </button>
                  ...
                  {isOpen && (
                    <div className={styles.blockTopics}>
                        ...
                        return (
                          <button key={topic.id}
                            ...
                            <div className={styles.topicRowRight}>
                              {hlCount > 0 && <span className={styles.topicHlBadge}><Highlighter size={9} /> {hlCount}</span>}
                      ...
                      <div className={styles.blockTopicsFooter}>
                        <button className={styles.practiceBtn} style={{['--bc' as string]:block.color,['--bb' as string]:block.bg}}
                          onClick={() => onSelectMode(block.id, block.label)}>
