...
function Sparkline({ data = [] as number[], color = '#2563EB', width = 80, height = 32 }) {
  ...
  return (
    <svg width={width} height={height} style={{ overflow: 'visible' }}>
      ...
      <path d={d} fill="none" stroke={`url(#spGrad_${color.replace('#', '')})`} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
...
// ── ActionBanner ───────────────────────────────────────────────────────────
function ActionBanner({ msg, type, onClose }: { msg: string; type: string; onClose: () => void }) {
  if (!msg) return null
...
interface AcademiaFormData {
  name: string; slug: string; plan: string; contact_email: string; contact_phone: string
...
function ModalAcademia({ academia, onGuardar, onClose }: {
  academia?:  any
  onGuardar:  (d: AcademiaFormData) => Promise<{ error?: string }>
  onClose:    () => void
}) {
  const isEdit = !!academia
  const [form, setForm] = useState<AcademiaFormData>({
    name:            academia?.name            ?? '',
    slug:            academia?.slug            ?? '',
    plan:            academia?.plan            ?? 'starter',
  ...
  const set = (k: keyof AcademiaFormData, v: string) => {
  ...
  return (
    <Modal title={isEdit ? `Editar — ${academia.name}` : 'Nueva academia'} onClose={onClose} wide>
      <div className={styles.modalTabs}>
        {tabs.map(t => (
          <button key={t.id} className={[styles.modalTab, tab === t.id ? styles.modalTabActive : ''].join(' ')} onClick={() => setTab(t.id)}>{t.label}</button>
        ))}
      </div>
      <div className={styles.modalBody}>
        {tab === 'basico' && (<>
          <Field label="Nombre de la academia"><input className={styles.input} value={form.name} onChange={e => set('name', e.target.value)} placeholder="Ej: Academia Opositas Madrid" autoFocus /></Field>
          {!isEdit && (<Field label="Slug (URL)" hint="Solo letras, números y guiones"><input className={styles.input} value={form.slug} onChange={e => set('slug', e.target.value)} placeholder="academia-opositas-madrid" /></Field>)}
          <Field label="Plan">
...
// ── ModalAsignatura ────────────────────────────────────────────────────────
function ModalAsignatura({ academia, onCrear, onClose }: { academia: any; onCrear: (d: any) => Promise<{ error?: string }>; onClose: () => void }) {
      ...
      <div className={styles.modalBody}>
        ...
        <Field label="Color">
          <div className={styles.colorPicker}>
            {COLORS.map(c => <button key={c} className={[styles.colorBtn, color === c ? styles.colorBtnActive : ''].join(' ')} style={{ ['--col' as string]: c }} onClick={() => setColor(c)}>{color === c && <Check size={11} />}</button>)}
          </div>
        </Field>
        {error && <div className={styles.errorMsg}><AlertTriangle size={13} />{error}</div>}
        <div className={styles.modalActions}>
...
// ── ModalUsuario ───────────────────────────────────────────────────────────
function ModalUsuario({ academia, onCrear, onClose }: { academia: any; onCrear: (d: any) => Promise<{ error?: string }>; onClose: () => void }) {
  const [username,  setUsername]  = useState('')
  const [password,  setPassword]  = useState('')
  const [role,      setRole]      = useState('alumno')
  const [subjectId, setSubjectId] = useState<string>(academia.subjects?.[0]?.id ?? '')
        ...
        {['profesor', 'alumno'].includes(role) && academia.subjects?.length > 0 && (
          <Field label="Asignatura">
            <select className={styles.select} value={subjectId} onChange={e => setSubjectId(e.target.value)}>
              {academia.subjects.map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
          </Field>
        )}
...
function KpiCard({ icon: Icon, label, value, color, sub, delay = 0, chart }: {
  icon: React.ElementType; label: string; value: number | string | null | undefined; color: string; sub?: string; delay?: number; chart?: number[]
}) {
  const displayed = useCountUp(typeof value === 'number' ? value : null)
  return (
    <motion.div className={styles.kpiCard} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay, duration: 0.4 }} style={{ ['--kc' as string]: color }}>
      <div className={styles.kpiTop}>
        <div className={styles.kpiIconWrap} style={{ background: `${color}18`, border: `1px solid ${color}30` }}><Icon size={16} style={{ color }} /></div>
...
// ── HeroCard ───────────────────────────────────────────────────────────────
function HeroCard({ stats }: { stats: any }) {
  const mrr = useCountUp(stats?.mrr ?? 0)
  const arr = Math.round((stats?.mrr ?? 0) * 12)
  return (
    <motion.div className={styles.heroCard} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
      ...
      <div className={styles.heroCardStats}>
        <div className={styles.heroStatItem}><span className={styles.heroStatNum}>{stats?.acadActivas ?? 0}</span><span className={styles.heroStatLabel}>academias activas</span></div>
        <div className={styles.heroStatDivider} />
        <div className={styles.heroStatItem}><span className={styles.heroStatNum}>{stats?.totalAlumnos ?? 0}</span><span className={styles.heroStatLabel}>alumnos totales</span></div>
        <div className={styles.heroStatDivider} />
        <div className={styles.heroStatItem}><span className={styles.heroStatNum}>{stats?.sesiones30d ?? 0}</span><span className={styles.heroStatLabel}>sesiones 30d</span></div>
      </div>
    </motion.div>
  )
}
...
// ── PaymentHealthCard ──────────────────────────────────────────────────────
function PaymentHealthCard({ stats }: { stats: any }) {
  ...
  const aldia      = Math.max(0, activas - morosas - pendientes)
  const pct        = Math.round((aldia / total) * 100)
  return (
    <motion.div className={styles.healthCard} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.5 }}>
      <div className={styles.healthTitle}>Estado de pago</div>
      <div className={styles.healthRows}>
        <div className={styles.healthRow}><span className={styles.healthDot} style={{ background: '#22C55E' }} /><span className={styles.healthRowLabel}>Al día</span><span className={styles.healthRowVal} style={{ color: '#22C55E' }}>{aldia}</span></div>
        <div className={styles.healthRow}><span className={styles.healthDot} style={{ background: '#F59E0B' }} /><span className={styles.healthRowLabel}>Pendiente</span><span className={styles.healthRowVal} style={{ color: '#F59E0B' }}>{pendientes}</span></div>
        <div className={styles.healthRow}><span className={styles.healthDot} style={{ background: '#EF4444' }} /><span className={styles.healthRowLabel}>Morosa</span><span className={styles.healthRowVal} style={{ color: '#EF4444' }}>{morosas}</span></div>
      </div>
      <div className={styles.healthBar}><div className={styles.healthBarFill} style={{ width: `${pct}%` }} /></div>
    ...
    <motion.div layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97 }}
      ...
      <button className={styles.acadHeader} onClick={() => setOpen(o => !o)}>
        ...
        <div className={styles.acadInfo}>
          <div className={styles.acadName}>{ac.name}{ac.suspended && <span className={styles.acadSuspTag}>SUSPENDIDA</span>}</div>
        ...
        <div className={styles.acadBadges}>
          ...
          <span className={styles.acadPlan} style={{ color: planCfg.color, background: planCfg.bg }}>{planCfg.label}</span>
          {ac.healthScore !== null && (
        ...
        <motion.div className={styles.acadChevron} animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.2 }}><ChevronDown size={14} /></motion.div>
      </button>
            ...
            <div className={styles.acadPanel}>
              ...
              <div className={styles.acadBento}>
                ...
                <div className={[styles.acadCell, styles.acadCellNote].join(' ')}>
                  ...
                  {ac.notes ? <span className={styles.acadNoteText}>{ac.notes}</span> : <span className={styles.acadCellEmpty}>Sin notas</span>}
                </div>
              </div>
              <div className={styles.acadActions}>
                ...
                <div className={styles.acadActionsR}>
                  <button className={[styles.acadActBtn, ac.suspended ? styles.acadActBtnGreen : styles.acadActBtnAmber].join(' ')} onClick={handleSuspender} disabled={suspending}>
                    {suspending ? <><RefreshCw size={12} className={styles.spinnerInline} /> Procesando…</> : ac.suspended ? <><PlayCircle size={12} /> Reactivar</> : <><PauseCircle size={12} /> Suspender</>}
                  </button>
...
export default function SuperadminPanel({ currentUser, modoPapelera = false }: { currentUser: CurrentUser | null; modoPapelera?: boolean }) {
  ...
  const [editarAcademia,  setEditarAcademia]  = useState<any>(null)
  const [modalAsignatura, setModalAsignatura] = useState<any>(null)
  ...
  const showBanner = (msg: string, type = 'success') => {
    setBannerMsg(msg); setBannerType(type)
    if (type === 'success') setTimeout(() => setBannerMsg(''), 6000)
  }

  const handleCrearAcademia   = useCallback(async (d: any) => crearAcademia(d),   [crearAcademia])
  const handleEditarAcademia  = useCallback(async (d: any) => actualizarAcademia(editarAcademia?.id, d), [actualizarAcademia, editarAcademia])
  const handleCrearAsignatura = useCallback(async (d: any) => crearAsignatura(d), [crearAsignatura])
  const handleCrearUsuario    = useCallback(async (d: any) => crearUsuario(d),    [crearUsuario])
  const handleToggleSuspender = useCallback(async (id: string, suspendido: boolean) => {
  ...
  const papelera       = academias.filter((ac: any) => !!ac.deleted_at)
  const acadsActivas   = academias.filter((ac: any) => !ac.deleted_at)
  const acadsFiltradas = acadsActivas.filter((ac: any) => {
      ...
      <div className={styles.inner}>
        <div className={styles.pageHeader}>
          ...
          <div className={styles.headerActions}>
            ...
            {!verPapelera && (
              <motion.button className={styles.btnPrimary} onClick={() => setModalAcademia(true)} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>
        ...
        {!verPapelera && (<>
          ...
          <div className={styles.acadSection}>
            <div className={styles.acadSectionHeader}>
              <h2 className={styles.acadSectionTitle}>Academias <span className={styles.acadSectionCount}>{acadsActivas.length}</span></h2>
              <div className={styles.filtros}>
                {[
                  { id: 'todas',       label: 'Todas' },
                  { id: 'activas',     label: `Activas (${acadsActivas.filter((a: any) => !a.suspended).length})` },
                  { id: 'suspendidas', label: `Suspendidas (${acadsActivas.filter((a: any) => a.suspended).length})` },
                  { id: 'morosas',     label: `Morosas (${acadsActivas.filter((a: any) => a.payment_status === 'overdue').length})` },
                ].map(f => (
                  <motion.button key={f.id} className={[styles.filtro, filtro === f.id ? styles.filtroActive : ''].join(' ')} onClick={() => setFiltro(f.id)} whileTap={{ scale: 0.95 }}>{f.label}</motion.button>
                ))}
              </div>
            </div>
            ...
            {acadsFiltradas.length === 0 ? (
              <motion.div className={styles.emptyAcads} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <Building2 size={36} strokeWidth={1.2} /><p>No hay academias en este filtro</p>
        ...
        {verPapelera && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
            <div className={styles.papeleraHeader}><Trash2 size={16} /><h2>Papelera <span>({papelera.length})</span></h2></div>
            {papelera.length === 0 ? (
              <div className={styles.papeleraEmpty}><Trash2 size={36} strokeWidth={1.2} /><p>La papelera está vacía</p></div>
            ) : (
              <div className={styles.acadList}>
                {papelera.map((ac: any) => (
                  <motion.div key={ac.id} className={styles.papeleraCard} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    <button className={styles.btnRestaurar} onClick={() => restaurarAcademia(ac.id)}><RotateCcw size={12} /> Restaurar</button>
                  </motion.div>
                ))}
              </div>
            )}
          ...
          {confirmDelete && (
            <motion.div className={styles.overlay} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div className={styles.modal} initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                exit={{ scale: 0.9, opacity: 0 }} transition={{ type: 'spring', damping: 22, stiffness: 300 }} onClick={e => e.stopPropagation()}>
                <div className={styles.modalHeader}>
