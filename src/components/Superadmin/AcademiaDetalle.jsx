...
function KpiCard({ icon: Icon, label, value, color, sub, dark, delay = 0 }) {
  ...
  return (
    <motion.div
      className={[styles.kpi, dark ? styles.kpiDark : ''].join(' ')}
      style={{ '--c': color }}
      ...
      <div className={styles.kpiTop}>
        <div className={styles.kpiIconWrap} style={{ background: dark ? 'rgba(255,255,255,0.08)' : (color + '18') }}>
          <Icon size={15} style={{ color: dark ? color : color }} />
        </div>
      </div>
      <div className={styles.kpiVal} style={{ color: dark ? (color === '#D7FE03' ? color : 'white') : 'var(--ink)' }}>
        {disp}
      </div>
      <div className={styles.kpiLabel} style={{ color: dark ? 'rgba(255,255,255,0.45)' : 'var(--ink-muted)' }}>
        {label}
      </div>
...
// ── Edge function call ────────────────────────────────────────────────────────
async function callGestionarUsuario(action, userId, params = {}) {
  try {
...
function UsuarioRow({ user, sesiones, emails, expanded, onToggle, onReload }) {
  ...
  const notaMedia    = userSessions.length
    ? Math.round(userSessions.reduce((a, s) => a + s.score, 0) / userSessions.length) : null
  const ultima       = userSessions[0]?.played_at
  const diasInactivo = ultima ? Math.floor((new Date() - new Date(ultima)) / 86400000) : null
  ...
  const handlePassword  = async () => {
    ...
    const ok = await run('cambiar_password', { password: newPassword })
    if (ok) closeModal()
  }
  const handleEmail = async () => {
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(newEmail.trim())) { setActionErr('Email no válido'); return }
    const ok = await run('editar_email', { email: newEmail.trim() })
    ...
    <>
          ...
          <Modal key="del" title="¿Eliminar usuario?" onClose={closeModal}>
            <div className={styles.modalBody}>
              <p className={styles.modalText}>
                Se borrará <strong>{user.username}</strong> de forma permanente.
              </p>
              ...
              <div className={styles.modalActions}>
                ...
                <button className={styles.modalBtnDanger} onClick={handleEliminar} disabled={loading}>
                  {loading ? <RefreshCw size={13} className={styles.spinnerInline} /> : <Trash2 size={13} />}
                  Eliminar
                </button>
              </div>
            </div>
          ...
          <Modal key="pw" title={`Cambiar contraseña — ${user.username}`} onClose={closeModal}>
            <div className={styles.modalBody}>
              <div className={styles.modalField}>
                ...
                <div className={styles.pwWrap}>
                  <input type={showPw ? 'text' : 'password'} value={newPassword}
                    onChange={e => { setNewPassword(e.target.value); setActionErr('') }}
                    placeholder="Mínimo 6 caracteres" autoFocus className={styles.modalInput} />
                  <button type="button" onClick={() => setShowPw(v => !v)} className={styles.pwEye}>
              ...
              <div className={styles.modalField}>
                ...
                <input type={showPw ? 'text' : 'password'} value={newPassword2}
                  onChange={e => { setNewPassword2(e.target.value); setActionErr('') }}
                  placeholder="Repite la contraseña" className={styles.modalInput} />
              </div>
              ...
              <div className={styles.modalActions}>
                ...
                <button className={styles.modalBtnPrimary} onClick={handlePassword} disabled={loading}>
                  {loading ? <RefreshCw size={13} className={styles.spinnerInline} /> : <Check size={13} />}
                  Guardar
                </button>
              </div>
            </div>
            ...
            <div className={styles.modalBody}>
              <div className={styles.modalField}>
                ...
                <input type="email" value={newEmail}
                  onChange={e => { setNewEmail(e.target.value); setActionErr('') }}
                  placeholder="nuevo@email.com" autoFocus className={styles.modalInput} />
              </div>
              {actionErr && <div className={styles.modalErr}><AlertTriangle size={13} />{actionErr}</div>}
              <div className={styles.modalActions}>
                <button className={styles.modalBtnCancel} onClick={closeModal}>Cancelar</button>
      ...
      <motion.div
        ...
        className={[
          ...
          isAlumno && diasInactivo > 5 && !expirado ? styles.userCard2Risk : '',
        ].join(' ')}>

        <button className={styles.userCard2Header} onClick={onToggle}>
          <div className={styles.userCard2Left}>
            <div className={styles.userCard2Avatar} style={{ opacity: banned ? 0.5 : 1 }}>
            ...
            <div>
              ...
              <div className={styles.userCard2Badges}>
                ...
                {isAlumno && !banned && !expirado && diasInactivo > 5 && <span className={styles.chip} style={{ color:'#F59E0B', background:'rgba(245,158,11,0.1)' }}>⚠ Inactivo {diasInactivo}d</span>}
          ...
          <div className={styles.userCard2Right}>
            {isAlumno && notaMedia !== null && (
              <span className={styles.userCard2Score} style={{ color: scoreColor(notaMedia) }}>{notaMedia}%</span>
            )}
              ...
              <div className={styles.userCard2Body}>
                ...
                <div className={styles.userCard2Stats}>
                  {isAlumno && <>
                    ...
                    <div className={styles.userCard2Stat}><Clock size={12} />
                      <span>{diasInactivo === null ? 'Sin actividad' : diasInactivo === 0 ? 'Activo hoy' : `Hace ${diasInactivo}d`}</span>
                    </div>
                    ...
                    {email && <div className={styles.userCard2Stat}><Mail size={12} />
                    ...
                    {userSessions.slice(0, 7).reverse().map((s, i) => (
                      <div key={i} className={styles.miniBar}>
                        <div className={styles.miniBarFill}
                          style={{ height: `${s.score}%`, background: scoreColor(s.score) }} />
                ...
                <div className={styles.userCard2Actions}>
                  <button className={styles.userCard2Btn} onClick={() => { setNewEmail(email || ''); setModal('email') }}>
                    <Mail size={12} /> Editar email
                  </button>
                  <button className={styles.userCard2Btn} onClick={() => { setNewPassword(''); setNewPassword2(''); setModal('password') }}>
                    <Lock size={12} /> Cambiar contraseña
                  </button>
                  <button
                    className={[styles.userCard2Btn, banned ? styles.userCard2BtnActivar : styles.userCard2BtnWarn].join(' ')}
                    onClick={handleToggleBan} disabled={loading}>
                  ...
                  <button className={[styles.userCard2Btn, styles.userCard2BtnDanger].join(' ')}
                ...
                {actionErr && !modal && (
                  <div className={styles.userCard2Err}><AlertTriangle size={13} />{actionErr}</div>
                )}
              </div>
            </motion.div>
          )}
...
function SubjectSection({ subject, profiles, sesiones, emails, onReload }) {
  ...
  return (
    <div className={styles.subjectSection2} style={{ '--sc': subject.color }}>
      <button className={styles.subjectHeader2} onClick={() => setOpen(v => !v)}>
        ...
        <div className={styles.subjectInfo2}>
          ...
          <div className={styles.subjectMeta2}>
            ...
            {notaMedia !== null && (
              <span style={{ color: scoreColor(notaMedia) }}><BarChart2 size={11} /> {notaMedia}%</span>
            )}
          </div>
        </div>
          ...
          <motion.div
            ...
            <div className={styles.subjectBody2}>
              ...
              {profes.length === 0 && alumnos.length === 0 && (
                <p className={styles.empty2}>Sin usuarios en esta asignatura todavía</p>
              )}
            </div>
          </motion.div>
        )}
...
export default function AcademiaDetalle({ academia, onBack }) {
  ...
  const planColor    = { base: '#6366F1', medio: '#F59E0B', academia: '#10B981' }[plan] || '#6B7280'
  ...
  return (
    <div className={styles.page2}>
      ...
      <motion.div className={styles.heroHeader}
        ...
        <div className={styles.heroHeaderOrb1}/>
      <div className={styles.heroHeaderOrb2}/>
      <button className={styles.btnBack2} onClick={onBack}>
          <ArrowLeft size={14} /> Academias
        </button>

        <div className={styles.heroHeaderContent}>
          <div className={styles.heroAcadIcon}>
          ...
          <div>
            ...
            <div className={styles.heroAcadMeta}>
              ...
              <span className={styles.heroAcadPlan} style={{ color: planColor, background: planColor + '20' }}>
                Plan {plan}
              </span>
              {academia.suspended && (
                <span className={styles.heroAcadSuspended}>SUSPENDIDA</span>
              )}
            </div>
          </div>
        </div>
        ...
        <>
          ...
          <motion.div className={styles.bentoRow}
            ...
            <div className={styles.bentoCardDark}>
              ...
              <div className={styles.bentoPayStatus}
                style={{ color: academia.payment_status === 'active' ? '#D7FE03' : academia.payment_status === 'overdue' ? '#EF4444' : '#F59E0B' }}>
              ...
              {academia.contract_renews && (
                <div className={styles.bentoRenew}>
                  Renueva: {new Date(academia.contract_renews).toLocaleDateString('es-ES', { day:'2-digit', month:'short', year:'numeric' })}
                </div>
              )}
            </div>
            ...
            {(academia.billing_name || academia.billing_nif) && (
              <div className={styles.bentoCard}>
                ...
                {academia.payment_method && (
                  <div className={styles.bentoCardChip}>{academia.payment_method}</div>
                )}
              </div>
            )}
            ...
            {(academia.contact_email || academia.contact_phone || academia.city) && (
              <div className={styles.bentoCard}>
                ...
                {academia.contact_email && (
                  <a href={`mailto:${academia.contact_email}`} className={styles.bentoCardLink}>
                    {academia.contact_email}
                  </a>
                )}
            ...
            <div className={styles.bentoCard}>
              ...
              <div className={styles.bentoStatRow}>
                <div className={styles.bentoStatItem}>
                  <span className={styles.bentoStatNum}>{totalAlumnos}</span>
                  <span className={styles.bentoStatLabel}>alumnos</span>
                </div>
                <div className={styles.bentoStatItem}>
                  <span className={styles.bentoStatNum} style={{ color: '#D7FE03' }}>{activos}</span>
                  <span className={styles.bentoStatLabel}>activos 7d</span>
                </div>
                <div className={styles.bentoStatItem}>
                  <span className={styles.bentoStatNum}>{sesiones.length}</span>
                  <span className={styles.bentoStatLabel}>sesiones</span>
                </div>
              </div>
              {notaGlobal !== null && (
                <div className={styles.bentoNota} style={{ color: scoreColor(notaGlobal) }}>
                  {notaGlobal}% nota media global
                </div>
              )}
              {academia.notes && (
                <div className={styles.bentoNotes}>{academia.notes}</div>
              )}
            </div>
          </motion.div>
          ...
          {directores.length > 0 && (
            <motion.div className={styles.section2}
              ...
              <div className={styles.sectionHeader2}>
                <Building2 size={14} />
                <h2 className={styles.sectionTitle2}>Dirección</h2>
              </div>
          ...
          <motion.div className={styles.section2}
            ...
            <div className={styles.sectionHeader2}>
              <BookOpen size={14} />
              <h2 className={styles.sectionTitle2}>Asignaturas</h2>
            </div>
