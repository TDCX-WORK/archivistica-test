...
<div class="dates-row"><div class="date-block"><div class="date-label">Fecha de emisión</div><div class="date-val">${fmtDate(inv.created_at)}</div></div><div class="date-block"><div class="date-label">Vencimiento</div><div class="date-val">${fmtDate(inv.due_date)}</div></div><div class="date-block"><div class="date-label">Período</div><div class="date-val">${inv.period_start?`${fmtDate(inv.period_start)} – ${fmtDate(inv.period_end)}`:'—'}</div></div></div>
...
${inv.notes?`<div style="margin-top:16px;padding:14px;background:#f9fafb;border-radius:8px;font-size:11px;color:#6b7280"><strong>Notas:</strong> ${inv.notes}</div>`:''}
<div class="legal">Factura emitida de conformidad con el Real Decreto 1619/2012. IVA al tipo general del ${vatRate}% según la Ley 37/1992. En caso de discrepancia, contacte en el plazo de 30 días desde la fecha de emisión.</div>
</body></html>`
