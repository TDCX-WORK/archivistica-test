...
import type { CurrentUser, Question } from '../../../types'
...
function PreguntaCard({ pregunta, idx }: { pregunta: Question; idx: number }) {
  const [abierta, setAbierta] = useState(false)
  const opciones = Array.isArray(pregunta.options) ? pregunta.options as string[] : []
      ...
      {abierta && (
        <div className={styles.preguntaBody}>
          <div className={styles.opciones}>
            {opciones.map((op, i) => (
              <div key={i} className={[styles.opcion, i === pregunta.answer ? styles.opcionCorrecta : ''].join(' ')}>
                <span className={styles.opcionLetra}>{LETRAS[i]}</span>
                <span className={styles.opcionTexto}>{op}</span>
                {i === pregunta.answer && <span className={styles.opcionBadge}>✓ Correcta</span>}
              </div>
            ))}
          </div>
...
export default function BancoPreguntas({ currentUser, onLoad }: BancoPreguntasProps) {
  const [bloques,   setBloques]   = useState<Bloque[]>([])
  const [preguntas, setPreguntas] = useState<Question[]>([])
  const [loading,   setLoading]   = useState(true)
  const [bloqueSel, setBloqueSel] = useState<string>('todos')
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [{ data: bls }, { data: qs }] = await Promise.all([
        supabase.from('content_blocks').select('id, label, color, position').eq('subject_id', subjectId).order('position'),
        supabase.from('questions').select('id, question, options, answer, explanation, block_id, topic_id, subject_id, academy_id, difficulty, category').eq('subject_id', subjectId).eq('academy_id', academyId).order('block_id'),
      ])
      setBloques((bls ?? []) as Bloque[])
      setPreguntas((qs ?? []) as Question[])
      setLoading(false)
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.filtros}>
        <button className={[styles.filtroBtn, bloqueSel === 'todos' ? styles.filtroActive : ''].join(' ')} onClick={() => setBloqueSel('todos')}>
      ...
      <div className={styles.searchWrap}>
        ...
        <input className={styles.searchInput} placeholder="Buscar pregunta…" value={busqueda} onChange={e => setBusqueda(e.target.value)} />
      </div>
