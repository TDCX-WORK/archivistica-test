...
function AvatarBig({ nombre, color = '#6366F1' }) {
  const iniciales = nombre
    ? nombre.split(' ').map(p => p[0]).slice(0, 2).join('').toUpperCase()
    : '?'
  return (
    <div className={styles.avatarBig} style={{ background: color + '22', color }}>
      {iniciales}
    </div>
  )
}
...
/* ─── Tab: Perfil ─────────────────────────────────────────────────────────── */
function TabPerfil({ currentUser, profile, saving, onSave }) {
  ...
  // Sync form cuando carga el perfil
  useEffect(() => {
    if (profile) setForm({
      full_name:     profile.full_name     || '',
      phone:         profile.phone         || '',
  ...
  const handleChangePass = async () => {
    if (!passForm.nueva || passForm.nueva !== passForm.confirmar) {
      setPassMsg({ error: true, text: 'Las contraseñas no coinciden' })
      return
    }
    if (passForm.nueva.length < 8) {
      setPassMsg({ error: true, text: 'Mínimo 8 caracteres' })
      return
    }
    setPassLoading(true)
    ...
    <div className={styles.tabContent}>
        ...
        <div className={styles.perfilHeaderInfo}>
          ...
          <div className={styles.perfilBadges}>
            ...
            {currentUser?.academyName && (
              <span className={styles.academiaBadge}>{currentUser.academyName}</span>
            )}
          </div>
          {profile?.bio && !editando && (
            <p className={styles.perfilBio}>{profile.bio}</p>
          )}
        </div>
      ...
      {editando ? (
        <div className={styles.editCard}>
          <div className={styles.editGrid}>
            <div className={styles.editCampo}>
              ...
              <input className={styles.editInput} type="text" placeholder="Tu nombre y apellidos"
                value={form.full_name} onChange={e => setForm(p => ({ ...p, full_name: e.target.value }))} />
            </div>
            <div className={styles.editCampo}>
              <label className={styles.editLabel}>Teléfono</label>
              <input className={styles.editInput} type="tel" placeholder="612 345 678"
                value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} />
            </div>
            <div className={styles.editCampo} style={{ gridColumn: '1 / -1' }}>
              <label className={styles.editLabel}>Email de contacto</label>
              <input className={styles.editInput} type="email" placeholder="tu@email.com"
                value={form.email_contact} onChange={e => setForm(p => ({ ...p, email_contact: e.target.value }))} />
            </div>
            <div className={styles.editCampo} style={{ gridColumn: '1 / -1' }}>
              <label className={styles.editLabel}>Bio / presentación</label>
              <textarea className={styles.editTextarea} rows={3}
                placeholder="Cuéntale algo a tus alumnos sobre ti..."
                value={form.bio} onChange={e => setForm(p => ({ ...p, bio: e.target.value }))} />
            </div>
          </div>
          <button className={styles.btnGuardar} onClick={handleSave} disabled={saving}>
            {saving
              ? <><RefreshCw size={13} className={styles.spinner} /> Guardando…</>
        ...
        <div className={styles.infoCard}>
          ...
          {currentUser?.subjectName && (
            <InfoRow icon={BookOpen} label="Asignatura" value={currentUser.subjectName} />
          )}
        </div>
      )}
      ...
      <div className={styles.passCard}>
        <div className={styles.passCardHead}>
          ...
          <button className={styles.btnCambiarPass}
            onClick={() => { setChangingPass(v => !v); setPassMsg(null) }}>
        ...
        {changingPass && (
          <div className={styles.passForm}>
            <div className={styles.editCampo}>
              ...
              <input className={styles.editInput} type="password" placeholder="Mínimo 8 caracteres"
                value={passForm.nueva}
                onChange={e => setPassForm(p => ({ ...p, nueva: e.target.value }))} />
            </div>
            <div className={styles.editCampo}>
              <label className={styles.editLabel}>Confirmar contraseña</label>
              <input className={styles.editInput} type="password" placeholder="Repite la contraseña"
            ...
            <button className={styles.btnGuardar} onClick={handleChangePass} disabled={passLoading}>
              {passLoading
                ? <><RefreshCw size={13} className={styles.spinner} /> Actualizando…</>
                : <><Key size={13} /> Actualizar contraseña</>}
            </button>
          </div>
        )}
      </div>
...
/* ─── Tab: Plan semanal ───────────────────────────────────────────────────── */
function TabPlan({ currentUser, statsClase }) {
  ...
  return (
    <div className={styles.tabContent}>
      {/* Stats rápidas del profesor */}
      {statsClase && (
        <div className={styles.statsRow}>
          <div className={styles.statCard}>
            <Users size={16} className={styles.statIcon} style={{ color: '#0891B2' }} />
            <span className={styles.statVal}>{statsClase.totalAlumnos}</span>
            <span className={styles.statLabel}>Alumnos</span>
          </div>
          <div className={styles.statCard}>
            <Zap size={16} className={styles.statIcon} style={{ color: '#059669' }} />
            <span className={styles.statVal}>{statsClase.alumnosActivos}</span>
            <span className={styles.statLabel}>Activos 7d</span>
          </div>
          <div className={styles.statCard}>
            <BarChart2 size={16} className={styles.statIcon} style={{ color: '#7C3AED' }} />
            <span className={styles.statVal}>
              {statsClase.notaMediaClase !== null ? `${statsClase.notaMediaClase}%` : '—'}
            </span>
            <span className={styles.statLabel}>Nota media</span>
          </div>
          <div className={styles.statCard}>
            <CalendarDays size={16} className={styles.statIcon} style={{ color: '#D97706' }} />
            <span className={styles.statVal}>{statsClase.sesiones30d ?? 0}</span>
            <span className={styles.statLabel}>Sesiones 30d</span>
          </div>
        </div>
      )}
