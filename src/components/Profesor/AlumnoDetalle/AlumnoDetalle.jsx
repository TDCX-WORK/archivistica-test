...
/* ─── DonutMini ───────────────────────────────────────────────────────────── */
function DonutMini({ pct, color, size = 110, stroke = 11 }) {
...
/* ─── FalloItem expandible ────────────────────────────────────────────────── */
function FalloItem({ fallo, rank }) {
    ...
    <div className={[styles.falloItem, open ? styles.falloItemOpen : ''].join(' ')}>
      <button className={styles.falloHeader} onClick={() => setOpen(v => !v)}>
        ...
        <span className={styles.falloTexto}>
          {q?.question
            ...
            : <em className={styles.falloSinTexto}>Pregunta no disponible</em>}
        </span>
        <span className={styles.falloCount} style={{ color: fallo.fail_count >= 3 ? '#DC2626' : '#B45309' }}>
          {fallo.fail_count}× fallada
        </span>
        {open ? <ChevronUp size={14} className={styles.falloChevron} />
      ...
      {open && q && (
        <div className={styles.falloBody}>
          ...
          <p className={styles.falloMeta}>Próximo repaso: <strong>{formatFecha(fallo.next_review)}</strong></p>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className={styles.barChart}>
      {data.map((s, i) => {
        ...
        const date  = new Date(s.played_at || s.created_at)
...
function generateInformePDF(alumno, sesiones, temasLeidos, fallos) {
  ...
  <div class="hdr"><div><h1 style="font-size:22px;font-weight:800">Informe de progreso</h1><p style="font-size:15px;font-weight:700;margin-top:6px">${alumno.username}</p><p style="font-size:12px;color:#6B7280">Acceso hasta: ${formatFecha(alumno.accessUntil)}</p></div><div style="text-align:right"><p style="font-weight:700;text-transform:uppercase;font-size:10px;letter-spacing:.08em;color:#6B7280">FrostFox Academy</p><p style="font-size:11px;color:#6B7280;margin-top:4px">Generado el ${now}</p></div></div>
...
export default function AlumnoDetalle({ alumno, onBack, academyId, currentUser }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Traer texto real de las preguntas
      if (wrongs?.length) {
        const qIds = wrongs.map(f => f.question_id).filter(Boolean)
        const { data: pregs } = await supabase
          .from('questions').select('id, question, options, answer, explanation')
          .in('id', qIds)
        const map = {}
        for (const q of (pregs || [])) map[q.id] = q
        setFallos(wrongs.map(f => ({ ...f, question: map[f.question_id] || null })))
      } else {
        setFallos([])
      }
      setLoading(false)
      ...
      <div className={styles.pageHeader}>
        ...
        <div className={styles.alumnoTitle}>
          <div className={styles.alumnoAvatar}
            style={{ background: scoreColor(alumno.notaMedia) + '22', color: scoreColor(alumno.notaMedia) }}>
          ...
          <div>
            ...
            <div className={styles.alumnoMeta}>
              <span className={styles.estadoBadge}
                style={{ color: estadoAlumno.color, background: estadoAlumno.bg }}>
                {estadoAlumno.label}
              </span>
            ...
            <div className={styles.card}>
              ...
              <div className={styles.miniStats}>
                <div className={styles.miniStat}>
                  <span className={styles.miniStatVal}>{sesiones.length}</span>
                  <span className={styles.miniStatLabel}>sesiones</span>
                </div>
                <div className={styles.miniStat}>
                  <span className={styles.miniStatVal}>{temasLeidos.length}</span>
                  <span className={styles.miniStatLabel}>temas leídos</span>
                </div>
                <div className={styles.miniStat}>
                  <span className={styles.miniStatVal}>{alumno.racha}d</span>
                  <span className={styles.miniStatLabel}>racha</span>
                </div>
              </div>
            </div>
            ...
            <div className={styles.card}>
              ...
              <div className={styles.infoList}>
                <div className={styles.infoRow}>
                  <User size={13} className={styles.infoIcon} />
                  <span className={styles.infoLabel}>Usuario</span>
                  <span className={styles.infoVal}>{alumno.username}</span>
                </div>
                <div className={styles.infoRow}>
                  <Shield size={13} className={styles.infoIcon} />
                  <span className={styles.infoLabel}>Acceso hasta</span>
                  <span className={styles.infoVal}>{formatFecha(alumno.accessUntil) || 'Sin límite'}</span>
                </div>
                <div className={styles.infoRow}>
                  <Clock size={13} className={styles.infoIcon} />
                  <span className={styles.infoLabel}>Última actividad</span>
                  <span className={styles.infoVal}>{ultimaActividad}</span>
                </div>
                <div className={styles.infoRow}>
                  <Flame size={13} className={styles.infoIcon} />
                  <span className={styles.infoLabel}>Racha actual</span>
                  <span className={styles.infoVal}>{alumno.racha} día{alumno.racha !== 1 ? 's' : ''}</span>
                </div>
                {alumno.diasParaExpirar !== null && !alumno.accesoExpirado && (
                  <div className={styles.infoRow}>
                    ...
                    <span className={styles.infoVal}
                      style={{ color: alumno.diasParaExpirar <= 7 ? '#DC2626' : 'inherit' }}>
        ...
        <div className={styles.msgOverlay} onClick={() => setShowMsgModal(false)}>
          <div className={styles.msgModal} onClick={e => e.stopPropagation()}>
            ...
            {msgSent ? (
              ...
              <>
                <textarea className={styles.msgTextarea}
                ...
                <div className={styles.msgModalFoot}>
                  ...
                  <button className={styles.msgBtnEnviar} onClick={handleEnviarMensaje}
