...
interface RichFeedbackProps {
  question:      SupuestoQuestion
  selectedIndex: number | undefined
}

function RichFeedback({ question, selectedIndex }: RichFeedbackProps) {
...
export default function SupuestoRunner({ supuesto, onGoHome }: SupuestoRunnerProps) {
  ...
  const correct = Object.entries(answers).filter(([i, a]) => (supuesto.questions ?? [])[Number(i)]?.answer === a).length
        ...
        <main className={styles.qPanel}>
          <div className={styles.qHeader}>
            ...
            <div className={styles.qBar}>
              <div className={styles.qBarFill} style={{ width: `${((index+1)/totalQ)*100}%` }} />
            </div>
          </div>
          ...
          {answered && q && <RichFeedback question={q} selectedIndex={answers[index]} />}
