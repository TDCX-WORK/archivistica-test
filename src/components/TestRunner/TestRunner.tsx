...
function isUUID(id: string): boolean { return !!id && id.includes('-') && !modes[id] }

interface BlockInfo { label: string; color: string }

interface FetchResult { questions: Question[]; fromTopic: boolean }
...
async function fetchQuestions(
  ...
  const toQ = (data: unknown[] | null) => (data ?? []) as Question[]
  ...
  if (modeId === 'quick_review') {
    ...
    const { data } = await q
    return { questions: shuffle(toQ(data)), fromTopic: false }
  }
  ...
  if (isUUID(modeId)) {
    if (topicId) {
      ...
      if (topicData && topicData.length >= 5) {
        return { questions: shuffle(toQ(topicData)).slice(0, 20), fromTopic: true }
      }

      const topicQuestionIds = new Set((topicData ?? []).map((q: { id: string }) => q.id))
      ...
      const blockExtras = toQ(blockData).filter(q => !topicQuestionIds.has(q.id))
      const combined    = [...toQ(topicData), ...shuffle(blockExtras)]
    ...
    if (subjectId) q = q.eq('subject_id', subjectId)
    const { data } = await q
    return { questions: shuffle(toQ(data)).slice(0, 20), fromTopic: false }
  }
  ...
  return { questions: shuffle(toQ(data)).slice(0, mode.questions), fromTopic: false }
}
...
export default function TestRunner({
    ...
    : isFailMode
      ...
      : isTopicMode
        ...
        : isBlockMode
          ...
          : (modes[modeId]?.label || 'Test')

  const totalSecs   = isFailMode ? null : isBlockMode ? 25 * 60 : (modes[modeId]?.timeMinutes ?? 25) * 60
  const timeMinutes = isBlockMode ? 25 : modes[modeId]?.timeMinutes

  const [blockMap,  setBlockMap]  = useState<Record<string, BlockInfo>>({})
  const [phase,     setPhase]     = useState<'intro' | 'running' | 'finished'>(isQuickReview ? 'running' : 'intro')
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      for (const b of (data ?? []) as { id: string; label: string; color: string }[]) map[b.id] = { label: b.label, color: b.color }
      setBlockMap(map)
    }
    load()
  }, [academyId, subjectId])
  ...
  useEffect(() => {
    ...
    if ((secsLeft ?? 0) <= 0) { handleFinish(); return }
    const id = setInterval(() => setSecsLeft(s => (s ?? 0) - 1), 1000)
  ...
  const currentBlockTag = current?.block_id && blockMap[current.block_id]
    ? blockMap[current.block_id]!.label : null
    ...
    <div className={styles.intro}>
      <div className={styles.introCard}>
        ...
        <p className={styles.introDesc}>
          ...
          {!isFailMode && !isBlockMode && (modes[modeId]?.description || `${qCount} preguntas`)}
        </p>
  ...
  if (phase === 'finished') return (
    <Results
      questions={questions} answers={answers} onGoHome={onGoHome}
  ...
  const mm     = totalSecs ? String(Math.floor((secsLeft ?? 0) / 60)).padStart(2, '0') : null
  const ss     = totalSecs ? String((secsLeft ?? 0) % 60).padStart(2, '0') : null
  const urgent = totalSecs && (secsLeft ?? 0) < 120
